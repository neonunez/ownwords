...
export async function ingestCourseVersion(
  ...
  if (existingVersion) {
    if (existingVersion.status === "draft" && existingVersion.content_hash === hash) {
      return { courseId: pack.course.id, version: pack.version, contentHash: hash, outcome: "unchanged" };
    }
    throw new ContentTransitionError(
      existingVersion.status === "published" ? "PUBLISHED_VERSION_IMMUTABLE" : "DRAFT_VERSION_EXISTS",
  ...
  const maxVersion = await first<MaxVersionRow>(
    db.prepare("SELECT MAX(version) AS max_version FROM learning_course_versions WHERE course_id = ?")
      .bind(pack.course.id),
  );
  ...
  for (const unit of pack.units) {
    statements.push(
      ...
      ).bind(pack.course.id, pack.version, unit.id, unit.position, unit.title, unit.canDo),
    );
    for (const lesson of unit.lessons) {
      statements.push(
        db.prepare(
          "INSERT INTO learning_lessons (course_id, course_version, lesson_id, unit_id, position, title) VALUES (?, ?, ?, ?, ?, ?)",
        ).bind(pack.course.id, pack.version, lesson.id, unit.id, lesson.position, lesson.title),
      );
      for (const step of lesson.steps) {
        statements.push(
          db.prepare(
            "INSERT INTO learning_steps (course_id, course_version, step_id, lesson_id, position, kind, payload_json) VALUES (?, ?, ?, ?, ?, ?, ?)",
          ).bind(pack.course.id, pack.version, step.id, lesson.id, step.position, step.kind, json(step.payload)),
        );
      }
    }
  }
      ...
      for (const step of lesson.steps) {
        for (const item of step.items) {
          statements.push(
            ...
            ).bind(pack.course.id, pack.version, step.id, item.itemId, item.role, item.position),
          );
        }
      }
    }
  ...
  if (results.some((result) => !result.success)) throw new Error("Atomic content import failed");
  return { courseId: pack.course.id, version: pack.version, contentHash: hash, outcome: "created" };
}
...
export async function publishCourseVersion(
  ...
  if (!current) throw new ContentTransitionError("VERSION_NOT_FOUND", "Content version does not exist");
  if (current.status !== "draft") {
    throw new ContentTransitionError("PUBLISHED_VERSION_IMMUTABLE", "Published content cannot transition again");
  }
  const result = await db.prepare(
    "UPDATE learning_course_versions SET status = 'published', published_at = ? WHERE course_id = ? AND version = ? AND status = 'draft'",
  ).bind(now.toISOString(), courseId, version).run();
  if (!result.success || result.meta.changes !== 1) {
    throw new ContentTransitionError("PUBLISH_CONFLICT", "Content version could not be published");
  }
}
...
export async function discardDraftCourseVersion(
  ...
  ).bind(courseId, version).run();
  if (!result.success || result.meta.changes !== 1) {
    throw new ContentTransitionError("DRAFT_NOT_FOUND", "Only an existing draft version may be discarded");
  }
}
