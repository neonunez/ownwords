...
export function string(
  ...
  if (trimmed.length < (options.min ?? 0) || trimmed.length > options.max) {
    throw new InputError(
      `${label} must be between ${options.min ?? 0} and ${options.max} characters`,
    );
  }
  return trimmed;
}
...
export function jsonRecord(
  ...
  if (JSON.stringify(result).length > 10_000)
    throw new InputError(`${label} is too large`);
  return result;
}

export function parseEquivalent(
  value: unknown,
  label = "equivalent",
): EquivalentInput {
  const input = object(value, label);
  const status = enumValue<TranslationStatus>(input.status, `${label}.status`, [
    "suggested",
    "confirmed",
    "waiting",
    "failed",
  ...
  if (status === "waiting" || status === "failed") {
    if (textValue !== undefined && textValue !== null) {
      throw new InputError(
        `${label}.text must be null for ${status} translations`,
      );
    }
  } else if (
    textValue === undefined ||
    ...
    throw new InputError(
      `${label}.text is required for ${status} translations`,
    );
  }
  return {
    ...
    status,
    text: textValue ?? null,
    fit: enumValue<FitLabel>(input.fit ?? "exact", `${label}.fit`, [
      "exact",
      "broader",
      "narrower",
      "context_only",
...
export function parseSense(value: unknown, label = "sense"): SenseInput {
  const input = object(value, label);
  if (
    !Array.isArray(input.equivalents) ||
    input.equivalents.length < 1 ||
    input.equivalents.length > 30
  ) {
    throw new InputError(
      `${label}.equivalents must contain between 1 and 30 items`,
    );
  }
  return {
    gloss: optionalString(input.gloss, `${label}.gloss`, 500),
    note: optionalString(input.note, `${label}.note`, 2000),
    equivalents: input.equivalents.map((item, index) =>
      parseEquivalent(item, `${label}.equivalents[${index}]`),
    ),
  };
}
...
export const directions = [
  "recognize",
  "produce",
] as const satisfies readonly PracticeDirection[];
export const formats = [
  "flashcard",
  "cloze",
] as const satisfies readonly PracticeFormat[];
