...
describe("Lexicon HTTP ownership and CRUD", () => {
  it("requires identity and never accepts an owner from the request body", async () => {
    ...
    const response = await jsonRequest(
      ...
      {
        method: "POST",
        json: { ...verifiedEntryBody(), ownerId: "spoofed-user" },
      },
      ctx.db,
    );
  ...
  it("uses optimistic updates and stable soft deletion semantics", async () => {
    ...
    const updated = await jsonRequest(
      app,
      path,
      { method: "PATCH", json: { version: 1, note: "corrected by me" } },
      ctx.db,
    );
    ...
    const stale = await jsonRequest(
      app,
      path,
      { method: "PATCH", json: { version: 1, note: "stale" } },
      ctx.db,
    );
    ...
    for (const header of ["2", 'W/"2"', '"0"', '"2.5"']) {
      const unquoted = await jsonRequest(
        app,
        path,
        { method: "DELETE", headers: { "If-Match": header } },
        ctx.db,
      );
  ...
  it("rejects malformed and out-of-bounds payloads without internal details", async () => {
    ...
    const limit = await jsonRequest(
      app,
      "/api/v1/lexicon/entries?limit=101",
      {},
      ctx.db,
    );
...
describe("search, filtering, and pagination", () => {
  it("ignores case and diacritics but preserves distinct letters", async () => {
            ...
            senses: [
              {
                gloss: text,
                equivalents: [
                  { languageTag, text, status: "manual", fit: "exact" },
                ],
              },
            ],
          },
    ...
    const camion = await jsonRequest(
      app,
      "/api/v1/lexicon/entries?query=CAMION",
      {},
      ctx.db,
    );
    assert.equal((((await camion.json()) as any).data as any[]).length, 1);
    const canon = await jsonRequest(
      app,
      "/api/v1/lexicon/entries?query=canon",
      {},
      ctx.db,
    );
    assert.equal((((await canon.json()) as any).data as any[]).length, 0);
    const slovo = await jsonRequest(
      app,
      `/api/v1/lexicon/entries?query=${encodeURIComponent("слово")}`,
      {},
      ctx.db,
    );
    assert.equal((((await slovo.json()) as any).data as any[]).length, 1);
    const vse = await jsonRequest(
      app,
      `/api/v1/lexicon/entries?query=${encodeURIComponent("все")}`,
      {},
      ctx.db,
    );
    ...
    const search = async (query: string): Promise<string[]> => {
      const response = await jsonRequest(
        app,
        `/api/v1/lexicon/entries?query=${encodeURIComponent(query)}`,
        {},
        ctx.db,
      );
      return (((await response.json()) as any).data as any[]).map(
        (entry) => entry.senses[0].equivalents[0].text,
      );
    };
  ...
  it("stores BCP 47-style language tags in canonical casing", async () => {
      ...
      "/api/v1/lexicon/entries",
      {
        ...
        json: {
          ...
          senses: [
            {
              equivalents: [
                { languageTag: "pt-br", text: "bom dia", status: "manual" },
              ],
            },
          ],
        },
    ...
    assert.equal(response.status, 201);
    assert.equal(
      ((await response.json()) as any).data.senses[0].equivalents[0]
        .languageTag,
      "pt-BR",
    );
  });
  ...
  it("returns bounded cursor pages and verification filters", async () => {
      ...
      senses: [
        {
          gloss: "unverified",
          equivalents: [
            { languageTag: "ru", text: "тест", status: "suggested" },
          ],
        },
      ],
    });
    ...
    const firstPage = await jsonRequest(
      app,
      "/api/v1/lexicon/entries?limit=1",
      {},
      ctx.db,
    );
    ...
    const unverified = await jsonRequest(
      app,
      "/api/v1/lexicon/entries?verification=unverified",
      {},
      ctx.db,
    );
  ...
  it("applies mastery to the selected language instead of another equivalent", async () => {
    ...
    const russian = await jsonRequest(
      app,
      "/api/v1/lexicon/entries?language=ru&mastery=mastered",
      {},
      ctx.db,
    );
    assert.equal(((await russian.json()) as any).data.length, 0);
    const english = await jsonRequest(
      app,
      "/api/v1/lexicon/entries?language=en&mastery=mastered",
      {},
      ctx.db,
    );
  ...
  it("exposes per-direction mastery on each equivalent and filters mastery by direction", async () => {
    const ctx = await context();
    const entry = await createVerifiedEntry(ctx);
    const initial = entry.senses[0].equivalents.find(
    ...
    const equivalents = ((await list.json()) as any).data[0].senses[0]
      .equivalents as any[];
    assert.deepEqual(
      equivalents.find((item) => item.languageTag === "ru").mastery,
      {
        recognize: {
          level: "mastered",
    ...
    assert.equal(
      await filter("language=ru&mastery=mastered&direction=recognize"),
      1,
    );
    assert.equal(
      await filter("language=ru&mastery=mastered&direction=produce"),
      0,
    );
    assert.equal(
      await filter("language=ru&mastery=due&direction=recognize"),
      0,
    );
    assert.equal(await filter("language=ru&mastery=due&direction=produce"), 1);
    const missingMastery = await jsonRequest(
      app,
      "/api/v1/lexicon/entries?direction=produce",
      {},
      ctx.db,
    );
    ...
    assert.equal(
      ((await suggested.json()) as any).data.senses[0].equivalents[0].mastery,
      null,
    );
  });
});
...
describe("validated cloze and answer matching", () => {
  it("returns honest unavailable state and requires explicit validated cloze data", async () => {
    ...
    const invalid = await jsonRequest(
      app,
      base,
      { method: "POST", json: { template: "Привет!", answer: "привет" } },
      ctx.db,
    );
  ...
  it("stops serving answer checks after an equivalent becomes a false friend", async () => {
    ...
    const created = await jsonRequest(
      ...
      {
        method: "POST",
        json: { template: "{{blank}}, друг!", answer: "приве́т" },
      },
      ctx.db,
    );
    const cloze = ((await created.json()) as any).data;
    const reclassified = await jsonRequest(
      ...
      {
        method: "PATCH",
        json: { version: russian.version, fit: "false_friend" },
      },
      ctx.db,
    );
