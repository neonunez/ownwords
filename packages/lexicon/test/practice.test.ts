...
describe("practice eligibility and shared scheduler", () => {
  ...
  it("never schedules false friends or unverified Russian suggestions", async () => {
    ...
    const suggestion = sense.equivalents.find(
    ...
    const confirmed = await jsonRequest(
      ...
      {
        method: "PATCH",
        json: { version: suggestion.version, status: "confirmed" },
      },
      ctx.db,
    );
  ...
  it("keeps card identity and language filters in sync when an equivalent language changes", async () => {
    ...
    const updated = await jsonRequest(
      ...
      {
        method: "PATCH",
        json: { version: russian.version, languageTag: "uk" },
      },
      ctx.db,
    );
  ...
  it("does not create cards as a side effect of a stale confirmation", async () => {
    ...
    const stale = await jsonRequest(
      ...
      {
        method: "PATCH",
        json: { version: 1, status: "confirmed", languageTag: "uk" },
      },
      ctx.db,
    );
    ...
    const count = ctx.rawDb.sqlite
      .prepare(
        "SELECT COUNT(*) AS count FROM lexicon_practice_cards WHERE equivalent_id = ?",
      )
...
describe("practice prompts", () => {
  it("treats a blank gloss as absent instead of serving a blank prompt", async () => {
          ...
          senses: [
            {
              gloss: "   ",
              equivalents: [
                { languageTag: "ru", text: "дом", status: "manual" },
              ],
            },
          ],
        },
    ...
    assert.deepEqual((await due(ctx, "user-a", "session-a")).data[0].prompt, {
      type: "gloss",
      text: "house",
    });
...
describe("review idempotency, concurrency, and wrong-answer revisits", () => {
  it("replays duplicate submissions once and rejects an idempotency-key mismatch", async () => {
    ...
    const events = ctx.rawDb.sqlite
      .prepare(
        "SELECT COUNT(*) AS count FROM lexicon_review_events WHERE card_id = ?",
      )
  ...
  it("serializes concurrent distinct reviews and revisits a wrong answer later in the same session", async () => {
    ...
    assert.equal(
      later.data.some((item: any) => item.revisit === true),
      false,
    );
  });
  ...
  it("reports FSRS retention and next due time separately by direction", async () => {
    ...
    assert.deepEqual(
      unreviewed.map((item) => item.direction),
      ["recognize", "produce"],
    );
    assert.deepEqual(
      unreviewed.map((item) => item.retention),
      [null, null],
    );
    assert.deepEqual(
      unreviewed.map((item) => item.nextDueAt),
      [created, created],
    );
    assert.equal(
      unreviewed.some(
        (item) => "total" in item || "due" in item || "new" in item,
      ),
      false,
    );
