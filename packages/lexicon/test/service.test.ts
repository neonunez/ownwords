...
import type { IdGenerator, TranslationProvider, TranslationSuggestionRequest } from '../src/types.js';
...
class FakeProvider implements TranslationProvider {
  ...
  async suggest(request: TranslationSuggestionRequest) {
    ...
    return [{ text: 'предложение', fit: 'exact' as const, provenance: { fixture: true } }];
  }
}
...
describe('translation boundary and private cache', () => {
  it('defaults to a disabled provider that performs no request', async () => {
    ...
    await assert.rejects(provider.suggest({ sourceLanguage: 'en', targetLanguage: 'ru', text: 'hello' }), ProviderDisabledError);
  });

  it('caches per owner and never shares private context across accounts', async () => {
    const ctx = await context();
    const provider = new FakeProvider();
    const service = new TranslationSuggestionService(ctx.db, provider, ctx.clock);
    const request = {
      sourceLanguage: 'en',
      targetLanguage: 'ru',
  ...
  it('returns provider suggestions for review without overwriting a human correction', async () => {
    ...
    const english = sense.equivalents.find((item: any) => item.languageTag === 'en');
    ...
    const corrected = await jsonRequest(
      ...
      { method: 'PATCH', json: { version: russian.version, status: 'manual', text: 'здра́вствуй' } },
      ctx.db,
    );
    ...
    const suggestion = await jsonRequest(
      ...
      { method: 'POST', json: { sourceEquivalentId: english.id, targetLanguage: 'ru' } },
      ctx.db,
    );
    assert.equal(suggestion.status, 200);
    assert.equal((await suggestion.json() as any).data[0].text, 'предложение');

    const read = await jsonRequest(app, `/api/v1/lexicon/entries/${entry.id}`, {}, ctx.db);
    const current = (await read.json() as any).data.senses[0].equivalents.find((item: any) => item.id === russian.id);
  ...
  it('surfaces disabled suggestion service honestly', async () => {
    ...
    const response = await jsonRequest(
      appFor(ctx, 'user-a'),
      `/api/v1/lexicon/entries/${entry.id}/senses/${sense.id}/suggestions`,
      { method: 'POST', json: { sourceEquivalentId: source.id, targetLanguage: 'es' } },
      ctx.db,
    );
    assert.equal(response.status, 503);
    assert.equal((await response.json() as any).error.code, 'TRANSLATION_PROVIDER_DISABLED');
  });
});
...
describe('course integration and transaction failure behavior', () => {
    ...
    senses: [
      {
        ...
        equivalents: [
          { languageTag: 'en', text: 'hello', status: 'confirmed' as const, fit: 'exact' as const },
          { languageTag: 'ru', text: 'приве́т', status: 'confirmed' as const, fit: 'exact' as const },
        ],
      },
    ],
  };
  ...
  it('imports a course item atomically and idempotently under concurrent retry', async () => {
    const ctx = await context();
    const importer = createCourseLexiconImporter({ db: ctx.db, clock: ctx.clock, idGenerator: ctx.ids });
    ...
    const count = ctx.rawDb.sqlite.prepare('SELECT COUNT(*) AS count FROM lexicon_entries').get() as { count: number };
    assert.equal(count.count, 1);
    const marker = ctx.rawDb.sqlite.prepare('SELECT COUNT(*) AS count FROM lexicon_course_imports').get() as { count: number };
  ...
  it('separates the same course identity by owner', async () => {
    ...
    const second = await importer.importCourseEntry({ ...courseInput, ownerId: 'user-b' });
  ...
  it('rejects unverified course equivalents before any write', async () => {
    ...
    await assert.rejects(
      importer.importCourseEntry({
        ...courseInput,
        senses: [{ gloss: 'bad', equivalents: [{ languageTag: 'ru', text: 'может', status: 'suggested', fit: 'exact' }] }],
      }),
      /verified/u,
    );
    const count = ctx.rawDb.sqlite.prepare('SELECT COUNT(*) AS count FROM lexicon_entries').get() as { count: number };
  ...
  it('validates the typed course boundary at runtime before any write', async () => {
    ...
    await assert.rejects(
      importer.importCourseEntry({ ...courseInput, provenance: undefined } as any),
