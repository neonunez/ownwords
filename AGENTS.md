...
- Run `npm run check` and `npm run build` from the repository root before delivery. API tests use local Miniflare/D1 and require no provider credentials; `apps/api/scripts/backup-restore.test.mjs` and `production-hosting-routes.test.mjs` also spawn real `wrangler dev` servers (the latter builds the app first).
...
- Course content reaches production only through the guarded operator route in `apps/api/src/contentPublication.ts`, driven by `npm run content:publish:remote --workspace @ownwords/api` (`scripts/remote-publication.mjs` holds the target guards); it reuses the Learning operator importer, preflights by default, and has never been run remotely. The exact command, its `CONTENT_PUBLISH_TOKEN` secret and the immutability consequence are in `docs/SETUP.md`.
