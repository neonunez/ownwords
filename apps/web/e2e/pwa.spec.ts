...
test.describe("installing, and the offline shell", () => {
  test("serves a manifest that installs as a standalone app", async ({
    page,
    request,
  }) => {
    await page.goto("/");
  ...
  test("declares the iOS home-screen metadata a PWA needs there", async ({
    page,
  }) => {
    await page.goto("/");
    await expect(
  ...
  test("opens its shell again with the network cut", async ({
    page,
    context,
  }) => {
  ...
  test("draws its own type offline, rather than fetching a webfont", async ({
    page,
  }) => {
  ...
  test("clears older caches, so no stale shell can survive a new build", async ({
    page,
  }) => {
    await page.goto("/");
