...
test.describe("the core flows", () => {
  test("captures an entry, reviews its translations and saves it", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/stays out of practice until you confirm it/),
    ).toBeVisible();

    await page
      .getByRole("button", { name: /Confirm the Español equivalent/ })
      .click();
    ...
    await expect(
      page.getByRole("button", { name: /to let it slide/ }),
    ).toBeVisible();
  });
  ...
  test("fixes a translation from the entry sheet", async ({ page }) => {
    ...
    await expect(
      page.locator("#ow-main").getByText("false friend"),
    ).toBeVisible();
  });
  ...
  test("gives the prompt before the answer in practice", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "Check your answer" }).click();
    await expect(
      page.getByText(/It comes back later this session\./),
    ).toBeVisible();
  });
  ...
  test("turns a flashcard over and rates it", async ({ page }) => {
    ...
    await expect(
      page.getByRole("radiogroup", { name: "Practice format" }),
    ).toHaveCount(0);
  ...
  test("walks a lesson from reading it to the contrast drill", async ({
    ...
    await expect(
      page.getByText(/Right\. That is “coffee, please”\./),
    ).toBeVisible();
  ...
  test("filters the alphabet to the letters that look Latin but are not", async ({
    ...
    await expect(
      page.getByRole("button", { name: /looks like B/ }),
    ).toBeVisible();
  });
  ...
  test("says which unit introduced each reference topic", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /Grammar and verbs/ }),
    ).toContainText("Unit 3");
