...
function rowsFor(
  ...
  return languages
    ...
    .map((language) => {
      const forLanguage = progress.filter(
        (entry) => entry.language === language.code,
      );
      const direction = (which: "recognise" | "produce") =>
        forLanguage.find((entry) => entry.direction === which)?.retention ??
        null;
      return {
        language,
  ...
  if (state.error || !state.data) {
    return (
      <>
        <TopBar title="Progress" large mode="Maintain" onMenu={openPanel} />
        <Failed
          message="Your progress could not be read. Nothing was lost."
      ...
      <Screen>
        {progress.estimate ? (
          <Card
            tone="accent"
            padding={18}
            onClick={() => navigate("/maintain/practice")}
          >
          ...
          <Card tone="soft" padding={18}>
            <p style={{ margin: 0, font: "var(--type-body)" }}>
              Nothing is due. The scheduler will pick the next words when they
              are close to slipping.
            </p>
          </Card>
        )}
            ...
            {rows.map((row, index) => (
              <div
                ...
                style={{
                  ...
                  padding: "14px 16px",
                  borderBottom:
                    index < rows.length - 1 ? "1px solid var(--border-1)" : 0,
                }}
              >
                <div
                  style={{
        ...
        <Section title="Coming up next">
          ...
          <Note>
            Retention is the only number here. There are no streaks, no points
            and no card counts.
          </Note>
        </Section>
      </Screen>
