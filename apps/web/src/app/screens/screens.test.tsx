...
describe("the Lexicon", () => {
  ...
  it("searches without the stress mark", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText("1 of 6 entries")).toBeInTheDocument(),
    );
  ...
  it("offers one action when nothing matches", async () => {
    ...
    expect(
      await screen.findByText(/Nothing in your Lexicon matches/),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Add “zzz”/ }),
    ).toBeInTheDocument();
  });
  ...
  it("offers three one-tap starter expressions when it is empty", async () => {
    renderScreen(<LexiconScreen />, {
      route: "/maintain/lexicon",
      demo: { entries: [] },
    });
    expect(
      await screen.findByText(/Your Lexicon is empty\./),
    ).toBeInTheDocument();
    for (const headword of ["no way", "it depends", "to let it slide"]) {
      expect(
        await screen.findByRole("button", { name: headword }),
      ).toBeInTheDocument();
    }
    await userEvent.click(screen.getByRole("button", { name: "no way" }));
    expect(await screen.findByText("1 entry")).toBeInTheDocument();
    expect(
      screen.queryByText(/Your Lexicon is empty\./),
    ).not.toBeInTheDocument();
  });
...
describe("an entry", () => {
  ...
  it("shows each sense with its equivalents, fit and state", async () => {
    ...
    expect(
      within(container).getByText("Sense 1 · in fact"),
    ).toBeInTheDocument();
  ...
  it("keeps the closed fit sheet out of reach", async () => {
    ...
    expect(
      document.querySelector('[role="dialog"]')?.closest("[inert]"),
    ).not.toBeNull();
  });
  ...
  it("records a fit chosen in the sheet", async () => {
    ...
    await userEvent.click(
      screen.getAllByRole("button", { name: /Fix the .* equivalent/ })[0]!,
    );
    const sheet = await screen.findByRole("dialog", {
      name: "How well does it fit?",
    });
    await userEvent.click(
      within(sheet).getByRole("button", { name: /narrower/i }),
    );
  ...
  it("adds another sense only once it has a gloss", async () => {
    ...
    await screen.findByText("actually");
    await userEvent.click(
      screen.getByRole("button", { name: "Add another sense" }),
    );
    const sheet = await screen.findByRole("dialog", {
      name: "Add another sense",
    });
    const save = within(sheet).getByRole("button", { name: "Add this sense" });
    expect(save).toBeDisabled();
    await userEvent.type(
      within(sheet).getByRole("textbox", { name: /What does it mean/ }),
      "   ",
    );
    expect(save).toBeDisabled();
    ...
    expect(
      await within(container).findByText("Sense 3 · as it happens"),
    ).toBeInTheDocument();
  });

  it("offers a retry when a translation failed, and says nothing was dropped", async () => {
    renderScreen(<EntryScreen />, {
      route: "/maintain/lexicon/e4",
      path: "/maintain/lexicon/:entryId",
    });
    expect(
      await screen.findByText("Translation failed. Nothing was dropped."),
    ).toBeInTheDocument();
    await userEvent.click(screen.getByRole("button", { name: "Retry" }));
    await waitFor(() =>
      expect(
        screen.queryByText("Translation failed. Nothing was dropped."),
      ).not.toBeInTheDocument(),
    );
  });
});
...
describe("adding an entry", () => {
  ...
  it("moves each candidate from waiting to suggested, then confirms it", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: /Confirm the Español equivalent/ }),
    );
...
describe("practice", () => {
  ...
  it("gives a prompt before the answer when the answer is wrong", async () => {
    ...
    expect(
      await screen.findByText(/It comes back later this session\./),
    ).toBeInTheDocument();
  });
  ...
  it("offers what is coming when the queue empties", async () => {
    ...
    expect(
      await screen.findByText("That is everything due."),
    ).toBeInTheDocument();
    await userEvent.click(
      screen.getByRole("button", { name: "Practise what is coming" }),
    );
    expect(
      await screen.findByText("after-dinner conversation"),
    ).toBeInTheDocument();
  ...
  it("reports retention and never a streak or a point", async () => {
    ...
    expect(
      screen.getByRole("img", {
        name: /Retention in Русский: recognise 22%, produce not practised yet/,
      }),
    ).toBeInTheDocument();
    expect(screen.queryByText(/streak/i)).toHaveTextContent(
...
describe("learn", () => {
  it("shows all 33 letters and filters to the Latin lookalikes", async () => {
    ...
    const list = await screen.findByRole("list", {
      name: "The Russian alphabet",
    });
    expect(within(list).getAllByRole("listitem")).toHaveLength(33);
    await userEvent.click(
      screen.getByRole("button", { name: "Look Latin, are not" }),
    );
    await waitFor(() =>
      expect(
        within(
          screen.getByRole("list", { name: "The Russian alphabet" }),
        ).getAllByRole("listitem").length,
      ).toBeLessThan(33),
    );
    expect(
      screen.getByRole("button", { name: /looks like B/ }),
    ).toBeInTheDocument();
  });
  ...
  it("shows the resume card, the units and the can-do milestones", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: /Continue · Unit 3/ }),
    ).toBeInTheDocument();
