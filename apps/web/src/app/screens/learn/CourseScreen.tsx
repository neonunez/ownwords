...
const marks: Record<UnitState, { icon: IconName; fg: string; bg: string; written: string }> = {
  done: {
    icon: 'check',
  ...
  current: { icon: 'play', fg: 'var(--fg-on-accent)', bg: 'var(--accent)', written: 'in progress' },
  locked: { icon: 'lock', fg: 'var(--fg-3)', bg: 'var(--bg-sunken)', written: 'locked' },
};
...
export function CourseScreen() {
  ...
  const header = <TopBar title="Course" large mode="Learn · Русский" onMenu={openPanel} />;
  ...
  if (state.error || !state.data) {
    return (
      <>
        {header}
        <Failed message="The course could not be read. Nothing was lost." onRetry={state.reload} />
      </>
    );
  }
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 16 }}>
            <span
              ...
              style={{
                ...
                height: 6,
                borderRadius: 99,
                background: 'color-mix(in oklab, var(--fg-on-accent) 22%, transparent)',
                display: 'block',
              }}
            >
              <span
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    minHeight: 64,
                    border: 0,
                    borderBottom: index < course.units.length - 1 ? '1px solid var(--border-1)' : 0,
                ...
                >
                  ...
                  <span style={{ font: 'var(--type-caption)', color: 'var(--fg-3)', textAlign: 'right' }}>
              ...
              {course.milestones.map((milestone) => (
                ...
                >
                  <Icon
                    ...
                    size={18}
                    color={milestone.reached ? 'var(--state-confirmed)' : 'var(--fg-3)'}
                  />
                  <span>
                    {milestone.text}
