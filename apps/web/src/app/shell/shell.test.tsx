...
describe('the tab bars', () => {
  ...
  it('gives Learn its own four, and never crosses modes', async () => {
    ...
    expect(screen.queryByRole('link', { name: 'Lexicon' })).not.toBeInTheDocument();
  });

  it('keeps the tab bar on a pushed screen, so no screen is a dead end', async () => {
    renderApp('/maintain/lexicon/e1');
    expect(await screen.findByRole('navigation', { name: 'Maintain' })).toBeInTheDocument();
  });
...
describe('navigating', () => {
  it('moves between tabs', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Lexicon', level: 1 })).toBeInTheDocument();
  });

  it('opens an entry from the Lexicon and comes back', async () => {
    renderApp('/maintain/lexicon');
    await userEvent.click(await screen.findByRole('button', { name: /to make do with/ }));
    expect(await screen.findByText('manage with what is available')).toBeInTheDocument();
    await userEvent.click(screen.getByRole('button', { name: 'Back to your Lexicon' }));
    expect(await screen.findByRole('heading', { name: 'Lexicon', level: 1 })).toBeInTheDocument();
  });
...
describe('the side panel', () => {
  it('holds mode, languages and preferences, and closes on Escape', async () => {
    renderApp();
    await userEvent.click(await screen.findByRole('button', { name: 'Open the side panel' }));
    const panel = await screen.findByRole('dialog', { name: 'Ownwórds' });
    expect(within(panel).getByRole('button', { name: /Maintain/ })).toHaveAttribute(
      'aria-current',
      'true',
    );
    expect(within(panel).getByText('Русский')).toBeInTheDocument();
    expect(within(panel).getByRole('switch', { name: 'Audio in the course' })).toBeInTheDocument();
  ...
  it('is the only way across to the other mode', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Course', level: 1 })).toBeInTheDocument();
    expect(await screen.findByRole('navigation', { name: 'Learn' })).toBeInTheDocument();
  });
});
