...
describe('state is always written in words', () => {
  ...
  it('says so when a direction has never been practised', () => {
    render(<MasteryMeter recognise={0.74} produce={null} label="Mastery in Español" />);
    expect(
      screen.getByRole('img', { name: 'Mastery in Español: recognise 74%, produce not practised yet' }),
    ).toBeInTheDocument();
  });
});
...
describe('controls are real controls', () => {
  ...
  it('hides the icon itself from assistive technology', () => {
    const { container } = render(<IconButton name="plus" label="Add an entry" />);
    expect(container.querySelector('svg')).toHaveAttribute('aria-hidden', 'true');
  });

  it('reports a chip as pressed when it is the chosen filter', () => {
    render(<Chip selected>Unverified</Chip>);
    expect(screen.getByRole('button', { name: 'Unverified' })).toHaveAttribute('aria-pressed', 'true');
  });

  it('exposes a switch with its state', async () => {
    const onChange = vi.fn();
    render(<Switch checked={false} label="Audio in the course" onChange={onChange} />);
...
describe('the segmented control', () => {
  ...
  it('is a radio group, not a tab list without panels', () => {
    render(
      <SegmentedControl label="Practice format" options={options} value="cloze" onChange={() => {}} />,
    );
    expect(screen.getByRole('radiogroup', { name: 'Practice format' })).toBeInTheDocument();
    expect(screen.getByRole('radio', { name: 'Complete the phrase' })).toBeChecked();
  });

  it('moves between options with the arrow keys', async () => {
    const onChange = vi.fn();
    render(
      <SegmentedControl label="Practice format" options={options} value="cloze" onChange={onChange} />,
    );
    await userEvent.tab();
  ...
  it('keeps one tab stop, so the group is passed in a single Tab', () => {
    ...
    expect(screen.getByRole('radio', { name: 'Complete the phrase' })).toHaveAttribute('tabindex', '0');
    expect(screen.getByRole('radio', { name: 'Flashcards' })).toHaveAttribute('tabindex', '-1');
  });
});
...
describe('navigation', () => {
  ...
  it('names the bar and marks the current tab', () => {
    render(<TabBar tabs={tabs} activeKey="lexicon" label="Maintain" onSelect={() => {}} />);
    expect(screen.getByRole('navigation', { name: 'Maintain' })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: 'Lexicon' })).toHaveAttribute('aria-current', 'page');
  });

  it('keeps tabs as links, so each carries a URL', () => {
    render(<TabBar tabs={tabs} activeKey="lexicon" label="Maintain" onSelect={() => {}} />);
    expect(screen.getByRole('link', { name: 'Progress' })).toHaveAttribute('href', '/maintain/progress');
  });

  it('gives each screen exactly one first-level heading', () => {
    const { rerender } = render(<TopBar title="Lexicon" large mode="Maintain" onMenu={() => {}} />);
