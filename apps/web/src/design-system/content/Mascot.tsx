...
export function Mascot({
  ...
  useEffect(() => {
    if (!tracking) return;
    const clamp = (value: number, max: number) => Math.max(-max, Math.min(max, value * max * 2.4));
    const onMove = (event: PointerEvent) => {
      ...
      setLook([
        ...
        clamp((event.clientY - (box.top + box.height * 0.46)) / box.height, 1.5),
      ]);
    };
  ...
  const eyes = happy ? (
    <g className="ow-kip-joy">
      <path d="M22.9 31.4c1.3-2.7 4.9-2.7 6.2 0" stroke={eye} strokeWidth="3" strokeLinecap="round" />
      <path d="M34.9 31.4c1.3-2.7 4.9-2.7 6.2 0" stroke={eye} strokeWidth="3" strokeLinecap="round" />
    </g>
  ) : (
  ...
  return (
    ...
    >
      <g className="ow-kip-body">
        ...
        <path className="ow-kip-acute" d="M32 9 41 4.2" stroke={color} strokeWidth="4.4" strokeLinecap="round" />
        <g
          className={thinking ? 'ow-kip-eyes ow-kip-look' : 'ow-kip-eyes'}
          style={thinking ? undefined : { transform: `translate(${offset[0]}px, ${offset[1]}px)` }}
        >
          {eyes}
        </g>
        {happy && (
          <g>
            <circle className="ow-kip-spark" cx="12.5" cy="19" r="1.9" fill={color} />
            <circle
